...
impl ControlApi<'_> {
    ...
    pub async fn step(&self, count: u32, wait: bool) -> Result<()> {
    ...
    pub async fn get_gamestate(&self) -> Result<StrDict> {
        let conn = self.bng.conn()?;
    ...
    pub async fn return_to_main_menu(&self) -> Result<()> {
        self.bng
            .conn()?
    ...
    /// Quit the simulator.
    pub async fn quit_beamng(&self) -> Result<()> {
